...
//import { IContest, IEntry, ICustomer, IState, IPagedResults, ICustomerResponse } from '../shared/interfaces';
...
//    baseContestUrl = this.baseUrl + 'contest';
...
////  getContest(page: number, pageSize: number, slateid: number, date: string): Observable<IPagedResults<IContest[]>> {
////    return this.http.get<IContest[]>(`${this.baseContestUrl}/page/${page}/${pageSize}`, { observe: 'response' })
////      .pipe(
////        map((res) => {
////          //Need to observe response in order to get to this header (see {observe: 'response'} above)
////          const xInlineCount = res.headers.get('X-InlineCount');
////          const totalRecords = Number(xInlineCount);
////          let contests = res.body as IContest[];
////         // this.calculateEntriesTotal(entries);
////          return {
////            results: contests,
////            totalRecords: totalRecords
////          };
